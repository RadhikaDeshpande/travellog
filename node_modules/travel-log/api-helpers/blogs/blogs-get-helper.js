  ...
  this.getPostsByUserProximity = function(req, res,callback) {
    ...
    MongoClient.connect(connection_string, function (err, db) {
      if(err) {
        ...
        var proximity_tbl  = db.collection('proximity_table'); // Temporary untill we move lat long country to blogs table
        var cursor = proximity_tbl.find({"country": userLatLongObj.country});
        var resultPostsArr = [];
        cursor.each(function(err, doc) {
          if(doc!=null) { 
            var dbLatLongObj = {};
