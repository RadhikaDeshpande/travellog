...
var blogsPostHelper = function() {
  ...
  */
            ...
            if(req.body.food_joint_name) { // only if food joint name is given 
              ...
              food_tbl.findOne({"_id": foodTblId}, function(err, doc) {
                if(!doc) { // New entry do an insert
                  ...
                  food_tbl.insert({"_id": foodTblId, "locationString":req.body.locationString, visitCount:1, likes:1, "foodPosts" :foodPostArr});
                } else {
  ...
  this.incTravelPostsLikeCount = function(req, callback) {

    var returnData = {};
    var returnMsg;
    var id = req.body.locationString;
    var bNo = +req.body.blogNumber;  // + converts string to integer number 
    //bNo = bNo.replace(/["']/g, "");
    ...
    MongoClient.connect(connection_string, function (err, db) {
      if(err) {
        ...
        var blog_tbl = db.collection('blogs_table');
        blog_tbl.update({ "_id": id, "blogs.blogNumber": bNo }, { $inc: { "blogs.$.viewCount" : 1 } });
        returnMsg = 'success';
        callback(returnMsg, null);
        return;
      }
  ...
  this.incFoodPostsLikeCount = function(req, callback) {

    var returnData = {};
    var returnMsg;
    var id = req.body.foodJointString;
    ...
    MongoClient.connect(connection_string, function (err, db) {
      if(err) {
        ...
        var food_tbl = db.collection('food_places_table');
        food_tbl.update({ "_id": id}, { $inc: { "likes" : 1 }});
        returnMsg = 'success';
        callback(returnMsg, null);
        return;
      }
  ...
  var _uploadToImageServer = function(imagesArray,callback) {
    ...
    keys.forEach(function(key) { // Loop through each column
      ...
      cloudinary.uploader.upload(filePath, function(cloudinaryResult) {
        ...
        imagesArray[key] = cloudinary.url(cloudinaryResult.public_id,{ width: 750, height: 450});
        count = +count + 1;
        if(count == len){
          callback(imagesArray);
        }
